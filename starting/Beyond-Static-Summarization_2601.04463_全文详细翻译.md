...
> **⚠️ v1 译文版本提示**
> - 本译文基于论文 **v1（2026-01-08 提交）** 的官方 HTML 逐段翻译。
> - 该论文已于 **2026-09-01 发布 v2**，并**大幅改写了实验章节**：v1 中 ProMem 是「完整性最高 + 问答最高」的双料第一，v2 中这两项第一均归新增基线 **MemOS**，ProMem 的定位改为「准确率与 F1 最高、在完整性与准确率之间平衡最好」。
> - 为便于对照，文末新增《附录：v2（2026-09-01）修订数据》一节，收录 v2 的主表、消融、小模型实验与跨基准结果。
> - **正文第 4 章（实验）所采用的 v1 数值，请以文末附录的 v2 数值为准。**
...
> ⚠️ **本节为 v1 数据。**该论文已于 2026-09-01 发布 v2 并大幅改写了实验章节，v2 数值见文末《附录：v2（2026-09-01）修订数据》。
...
> ⚠️ **本节（表 1）为 v1 数据。**该论文已于 2026-09-01 发布 v2 并大幅改写了实验章节，v2 主表见文末《附录：v2（2026-09-01）修订数据》。
> 表中为 v1 数值；v2 的 ProMem 为完整性 81.37 / 准确率 94.68 / QA 63.43，全表见文末附录。
...
**第一，我们的方法在记忆完整性上取得显著提升。** Mem0、Memobase、Supermemory 等基线的完整性只有约 41%–43%，而我们的方法达到 **73.80%**（v1；v2 提升到 81.37%，但最高完整性已由新增基线 MemOS 以 84.81% 取得，见文末附录）。这一结果有力证明：主动式记忆抽取框架能够成功找回被传统「一次性」抽取方法忽略的缺失细节。

**第二，在 v1 中 Memobase 的记忆准确率得分最高（92.24%）**，但值得注意的是它的**完整性极低（14.55%）**。这说明 Memobase 采取了**保守策略**——通过只抽极少的事实来保证高准确率，却漏掉了一半以上的重要信息。我们的方法取得了更好的平衡：在保持高准确率的同时捕获了远为完整的信息。v2 修订后准确率最高的是本文方法（94.68%），Memobase 退居其次。

**第三，在下游问答任务上，我们的方法取得最佳表现（62.26%）**，说明记忆的高完整性直接带来了更强的问答能力。（注：在 v1 中本文方法的下游问答表现最佳（62.26%）；**v2 修订后 QA 最高的是新增基线 MemOS（67.23%）**，本文方法为 63.43%，仍高于 LightMem（56.79%）。）

### 4.3 消融研究

为评估 ProMem 中每个模块的作用，我们做了消融实验，结果见**表 2**。其中「MC」指记忆补全模块（memory completion），「MV」指记忆验证模块（memory verification）。

> 下表为 v1 的消融（模块名 MC / MV）。v2 已把消融维度改为 CC / HV / DA / Rel 四模块 + One-off Extract 基线，数值见文末附录；其中 v1 的「w/o MC & MV」在 v2 中即 One-off Extract，三项数字 54.03 / 92.64 / 50.60 完全一致。
...
**第一，「w/o MC & MV」（v2 中改称 One-off Extract）变体展示了单遍抽取的局限**：它取得最高的记忆准确率，但完整性与问答分数最低。这说明**没有反馈时，智能体会采用「保守策略」**——保证正确性却漏掉大量细节。

**第二，两个模块对性能都不可或缺。** 把「w/o MC」与完整模型对比，记忆补全模块把记忆完整性从 60.33% 显著提升到 **73.80%**（v1；v2 对应维度下 CC 模块的贡献为 81.37% → 69.48%，见文末附录），证明其找回缺失信息的能力；而记忆验证模块（体现在「w/o MC & MV」与「w/o MC」的差距上）把问答准确率提升了 **10% 以上**，说明自我提问能有效帮助抽出与答案相关的事实。
...
**第一，ProMem 取得最佳表现**，问答准确率达到 69.57%（v1；v2 为 72.12%，对 LightMem 的 68.64%），超过所有基线，证明我们的主动式记忆抽取框架能有效捕获回答用户问题所需的关键信息。
...
## 附录：v2（2026-09-01）修订数据

> 本节收录论文 **v2（2026-09-01）** 相对 v1 的全部关键实验数据，供读者对照正文第 4 章（基于 v1）使用。所有数值逐字取自 v2 原文（arXiv:2601.04463v2，15 页），与 v1 译文的差异已逐项核对。**正文第 4 章的 v1 数值请以本节为准。**

### 一、v2 到底改了什么（叙事层面的变化）

v1 中 ProMem 是「完整性最高 + 问答最高」的双料第一；**v2 中这两项第一都归了新增基线 MemOS**，ProMem 的定位被改写为「准确率与 F1 最高、在完整性与准确率之间取得最佳平衡」。

v2 原文（4.2 节 Main Results）写道：

> "our method achieves the best overall memory quality. It reaches the highest memory accuracy and F1."
>
> "**MemOS achieves the highest memory integrity and QA accuracy.** However, its memory accuracy is relatively low. This means MemOS uses an aggressive strategy: it extracts many facts but introduces many errors. Memobase has good accuracy but very low integrity, meaning it misses too much information. In contrast, our method achieves the highest F1 score. We maintain the top accuracy while capturing very complete information (81.37% integrity)."

### 二、v2 主表（HaluMem，全量）

| 方法 | 记忆完整性 | 记忆准确率 | 记忆 F1 | 问答准确率 | # Tokens |
|---|---|---|---|---|---|
| Memobase | 14.55 | 92.24 | 25.13 | 35.33 | - |
| Supermemory | 41.53 | 90.32 | 56.89 | 54.07 | - |
| **MemOS**（v2 新增） | **84.81** | 86.25 | 79.70 | **67.23** | - |
| A-Mem（v2 新增） | - | - | - | 43.02 | 36.71K |
| MemoryOS（v2 新增） | - | - | - | 39.24 | 61.83K |
| Mem0 | 42.91 | 86.26 | 57.31 | 53.02 | 10.02K |
| LightMem (r=0.4) | 55.21 | 93.32 | 69.37 | 56.60 | 2.40K |
| LightMem (r=0.6) | 57.76 | 91.01 | 70.66 | 56.79 | 6.58K |
| LightMem (r=0.8) | 58.09 | 92.70 | 71.42 | 56.45 | 9.17K |
| LightMem | 57.94 | 92.33 | 71.19 | 55.81 | 10.56K |
| **ProMem** | **81.37** | **94.68** | **87.52** | 63.43 | 11.59K |

### 三、v1 → v2 逐项差异对照表

| 项目 | v1（正文现用） | v2（当前版本） | 影响 |
|---|---|---|---|
| ProMem 完整性 | 73.80 | 81.37 | 数字更新；且不再是最高（MemOS 84.81） |
| ProMem 准确率 | 89.47 | 94.68 | 数字更新；仍是最高 ✅ |
| ProMem QA | 62.26 | 63.43 | 数字更新；仍高于 LightMem（56.79）✅ |
| ProMem 新增指标 | — | 记忆 F1 87.52（最高） | 新增可引用论据 |
| MemOS | 未出现 | 84.81 / 86.25 / 79.70 / 67.23 | 新基线，在完整性与 QA 上均超 ProMem |
| A-Mem / MemoryOS | 未出现 | QA 43.02 / 39.24 | 新增基线 |
| LightMem | 无版本号，QA 56.60 | r=0.4/0.6/0.8 三档 + 默认档 | 需改为分档列出 |
| Token 成本 | 未列入主表 | 全表新增 # Tokens 列 | 新增可引用维度 |

### 四、v2 消融表（全量）

| 方法 | 记忆完整性 | 记忆准确率 | 问答准确率 | QA 幻觉 | QA 遗漏 |
|---|---|---|---|---|---|
| ProMem (Full) | 81.37 | 94.68 | 63.43 | 17.43 | 19.14 |
| w/o CC（去掉完整性检查） | 69.48 | 94.51 | 58.45 | 18.00 | 23.55 |
| w/o HV（去掉幻觉验证） | 80.84 | 93.43 | 60.35 | 21.40 | 18.25 |
| w/o DA（去掉细粒度细节） | 80.91 | 94.48 | 57.80 | 19.50 | 22.70 |
| w/o Rel（去掉关系边） | 81.15 | 94.42 | 61.12 | 18.40 | 20.48 |
| **One-off Extract** | **54.03** | **92.64** | **50.60** | 25.13 | 24.27 |

**说明**：
- v1 的「w/o MC & MV」在 v2 中改名为 **One-off Extract**，三项数字 **54.03 / 92.64 / 50.60 完全一致** ✅ —— 正文 4.3 节引用的这组数字仍可用，但术语要改。
- v2 的消融维度已从「MC / MV」改为 **CC（完整性检查）/ HV（幻觉验证）/ DA（细节锚定）/ Rel（关系验证）** 四模块 + One-off 基线。

### 五、v2 小模型实验（Table 3，Llama3.1-8B）
...
（与 v1 一致 ✅：正文 4.5 节写的「完整性从 Mem0 的 30.59% 提到 43.09%，问答分提升 10.74 个百分点」完全成立。）

### 六、v2 跨基准结果（Figure 5）

| 基准 | NativeRAG | Mem0 | LightMem | ProMem |
|---|---|---|---|---|
| LongMemEval-S | 65.09 | 43.31 | 68.64 | **72.12** |
| LoCoMo | 63.64 | 36.49 | 72.99 | **77.56** |

（正文 4.6 节现写「LongMemEval 69.57%」应改为 **72.12%**，对 LightMem 68.64 的结论方向不变。）

### 七、v2 实现细节

- 实验环境：单张 **NVIDIA A800**。
- 初始抽取与 probe 问题生成：**GPT-4o-mini**；多粒度记忆解耦：**Llama3.1-8B**；完整性检查（阈值 **τ_comp = 0.6**）与 QA 阶段 top-20 检索：**Qwen3-Embedding-8B**；NLI 幻觉验证：**roberta-large-mnli**；对话压缩：**LLMLingua-2**。
- ⚠️ v2 明确给出的可复用阈值**只有完整性检查阈值 0.6**；正文 4.1 节提到的「去重阈值 0.8」在 v2 正文中未再出现，请以 v2 原文为准。
